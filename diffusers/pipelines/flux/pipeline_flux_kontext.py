...
class FluxKontextPipeline(
    ...
    @staticmethod
    # Copied from diffusers.pipelines.flux.pipeline_flux.FluxPipeline._prepare_latent_image_ids
    def _prepare_multi_scale_latent_image_ids(batch_size, height, width, device, dtype, depth=None):

        latent_image_ids = torch.zeros(height, width, 3)
        latent_image_ids[..., 1] = latent_image_ids[..., 1] + torch.arange(height)[:, None] + 0.5
        latent_image_ids[..., 2] = latent_image_ids[..., 2] + torch.arange(width)[None, :] + 0.5


        if depth is not None:

            depth = depth.to(latent_image_ids.device)
            if depth.dim() == 2:
                depth_for_resize = depth.unsqueeze(0).unsqueeze(0)
            elif depth.dim() == 3:
                depth_for_resize = depth.unsqueeze(0)  # [C,H,W] -> [1,C,H,W]
            elif depth.dim() == 4:
                depth_for_resize = depth
            else:
                raise ValueError(f"Unsupported depth shape: {depth.shape}")


            if depth_for_resize.shape[-2:] != (height, width):
                depth_for_resize = F.interpolate(
                    depth_for_resize, size=(height, width),
                    mode='bilinear', align_corners=False
                )


            depth_resized = depth_for_resize.squeeze(0).squeeze(0)
            latent_image_ids[..., 0] += depth_resized



        H_news_W_news = [  (height // 16, width // 16),
                            (height // 8, width // 8),
                            (height // 4, width // 4)]
    
        pix_coords_tensor = latent_image_ids.unsqueeze(0).permute(0, 3, 1, 2)
        pix_coords_downs = []
        grid_level = 0
        for H_new, W_new in H_news_W_news:
            ...
            pix_coords_down[..., -2] = pix_coords_down[..., -2] / height * H_news_W_news[0][0] - 0.5
            pix_coords_down[..., -1] = pix_coords_down[..., -1] / width * H_news_W_news[0][1] - 0.5
            ...
            pix_coords_downs.append(pix_coords_down.to(device=device, dtype=dtype))

        return pix_coords_downs
    ...
    def prepare_latents(
        ...
        if input_image_latents is not None:
            image_latents= input_image_latents
            # image_latent_height, image_latent_width = image_latents.shape[2:]
            image_ids = self._prepare_multi_scale_latent_image_ids( #没用的
                batch_size, height * 8, width * 8, device, dtype, depth
            )
        else:
            ...
            if image is not None:
                ...
                if use_multi_scale_position:
                    if depth is None:
                        depth = torch.ones(image_latent_height * 8, image_latent_width * 8)
                    image_ids = self._prepare_multi_scale_latent_image_ids(
                        batch_size, image_latent_height * 8, image_latent_width * 8, device, dtype, depth
                    )
                    depth = None
                else:
        ...
        if use_multi_scale_position:
            latent_ids = self._prepare_multi_scale_latent_image_ids(batch_size, height * 8, width * 8, device, dtype, depth)
        else:
    ...
    def __call__(
        ...
        # if image is not None and not (isinstance(image, torch.Tensor) and image.size(1) == self.latent_channels):
            ...
            width = image_width // multiple_of * multiple_of
            height = image_height // multiple_of * multiple_of
        ...
        latents, image_latents, latent_ids, image_ids = self.prepare_latents(
            ...
            generator,
            latents,
            use_multi_scale_position,
            depth,
            input_image_latents,
        )
        if input_ids is not None:
            latent_ids = input_ids.to(device)


        if image_ids is not None:
            if input_img_ids is not None:

                if isinstance(input_img_ids, list):
                    input_img_ids = [x.to(device) for x in input_img_ids]
                    if use_multi_scale_position:

                        latent_ids = latent_ids + input_img_ids
                    else:
                        input_img_ids.insert(0, latent_ids)
                        latent_ids = input_img_ids
                else:
                    input_img_ids= input_img_ids.to(device)
                    latent_ids = torch.cat([latent_ids, input_img_ids], dim=0)  # dim 0 is sequence dimension
            else:
